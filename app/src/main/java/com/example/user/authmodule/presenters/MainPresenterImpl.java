...
import com.example.user.authmodule.model.Owner;
...
public class MainPresenterImpl extends BasePresenter<IBaseView.IMainView, IBaseInteractor.IInteractor> implements IPresenterContract.IMainPresenter {
    File file;
    private CallbackManager callbackManager;
    Owner mOwner;
    Event mEvent;

    public MainPresenterImpl(Application application, IBaseInteractor.IInteractor interactor, IValidator validator, INetworkCheck networkCheck, IRealmUtils realmUtils) {
        super();
...
//
//        realmUtils.deleteObject(1, UserRealm.class).subscribe((next) -> {
//            view.closeMain();
//        }, (err) -> {
...
//    }
    ...
    private void add(Event event) {
        realmUtils.addObject(event, Event.class).subscribe(next -> {
            if (next != null) {
    ...
    @Override
    public void doGetUsersList(Integer limit, Integer offset) {
